// ============================================================
// Proveedores.jsx — Gestión de proveedores + Cuenta Corriente + Historial
// Bulonera Miguel
// ============================================================
...
export default function Proveedores() {
  ...
  const [form, setForm] = useState({ nombre: '', cuit: '', direccion: '', telefono: '', email: '', tiene_cuenta_corriente: false })
  ...
  const abrirModalCrear = () => {
    ...
    setForm({ nombre: '', cuit: '', direccion: '', telefono: '', email: '', tiene_cuenta_corriente: false })
    setModalAbierto(true)
  }

  const abrirModalEditar = (p) => {
    setProveedorEditar(p)
    setForm({ nombre: p.nombre, cuit: p.cuit || '', direccion: p.direccion || '', telefono: p.telefono || '', email: p.email || '', tiene_cuenta_corriente: p.tiene_cuenta_corriente || false })
    setModalAbierto(true)
  }
  ...
  const guardarProveedor = async (e) => {
    e.preventDefault()
    try {
      const datos = { nombre: form.nombre, cuit: form.cuit || null, direccion: form.direccion || null, telefono: form.telefono || null, email: form.email || null, tiene_cuenta_corriente: form.tiene_cuenta_corriente }
      if (proveedorEditar) { await proveedoresApi.actualizar(proveedorEditar.id, datos) }
      else { await proveedoresApi.crear(datos) }
  ...
  const eliminarProveedor = async (id, nombre) => {
    ...
    try { await proveedoresApi.eliminar(id); cargarProveedores() }
    catch (e) { alert(e.message) }
  }

  const verHistorialProveedor = (proveedor) => {
    setProveedorHist(proveedor)
    setHistDesde(''); setHistHasta('')
    setHistCompras([]); setHistError(null)
    setCompraDetalle(null)
    setTab('historial-compras')
    cargarHistorial(proveedor.id)
  }

  const verCuentaCorriente = (proveedor) => {
    setProveedorCC(proveedor); setTab('cuenta-corriente')
  }
  ...
  useEffect(() => { if (tab === 'cuenta-corriente') cargarProveedoresCC() }, [tab])
  ...
  const cargarDetalle = async (proveedor, desde = '', hasta = '') => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/cuenta-corriente-proveedores/${proveedor.id}${p.toString() ? '?' + p.toString() : ''}`)
  ...
  const seleccionarProveedorCC = (proveedor) => {
    setProveedorCC(proveedor); setFiltroDesde(''); setFiltroHasta(''); setPagoOk(null)
    cargarDetalle(proveedor)
  }

  const volverAListaCC = () => { setProveedorCC(null); setDetalle(null); setPagoOk(null); cargarProveedoresCC() }
  ...
  const registrarPago = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/cuenta-corriente-proveedores/${proveedorCC.id}/pagos`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ monto: parseFloat(pago.monto), fecha: pago.fecha, medio_pago: pago.medio_pago, observaciones: pago.observaciones || null }) })
  ...
  const [proveedorHist, setProveedorHist]   = useState(null)
  const [histCompras, setHistCompras]       = useState([])
  ...
  const cargarHistorial = async (proveedorId, desde = '', hasta = '') => {
    try {
      setCargandoHist(true); setHistError(null)
      const p = new URLSearchParams()
      if (desde) p.append('desde', desde)
      if (hasta) p.append('hasta', hasta)
      const res = await fetch(`${BASE_URL}/api/compras/por-proveedor/${proveedorId}${p.toString() ? '?' + p.toString() : ''}`)
      if (!res.ok) throw new Error('Error al cargar historial')
      setHistCompras(await res.json())
    } catch (e) { setHistError(e.message) }
    finally { setCargandoHist(false) }
  }

  const seleccionarProveedorHist = (proveedor) => {
    setProveedorHist(proveedor)
    setHistDesde(''); setHistHasta('')
    setHistCompras([]); setHistError(null)
    setCompraDetalle(null)
  ...
  const volverAListaHist = () => {
    setProveedorHist(null); setHistCompras([])
    setHistDesde(''); setHistHasta('')
    setCompraDetalle(null)
  }
    ...
    <div className={styles.page}>
        ...
        <div className={styles.pageHeader}>
          <div>
            ...
            <span className={styles.pageSubtitle}>
              {tab === 'proveedores' ? 'Alta, edición y gestión de proveedores'
                : tab === 'cuenta-corriente' ? (proveedorCC ? `Cuenta corriente — ${proveedorCC.nombre}` : 'Deudas y pagos a proveedores')
                : proveedorHist ? `Historial de compras — ${proveedorHist.nombre}` : 'Seleccioná un proveedor para ver su historial'}
            </span>
          </div>
          {tab === 'proveedores' && <button className={styles.btnNuevo} onClick={abrirModalCrear}>+ Nuevo proveedor</button>}
          {tab === 'cuenta-corriente' && proveedorCC && <button className={styles.btnPagar} onClick={() => { setModalPago(true); setErrorPago(null) }}>+ Registrar pago</button>}
        </div>
        ...
        <div className={styles.tabs}>
          {TABS.map(t => (
            <button key={t.id}
              ...
              onClick={() => {
                setTab(t.id)
                if (t.id === 'proveedores') { setProveedorCC(null); setDetalle(null); setProveedorHist(null); setHistCompras([]) }
              }}>
              {t.label}
            </button>
          ))}
        ...
        {tab === 'proveedores' && (
          <div>
            ...
            <div className={styles.buscadorWrap}>
              <input type="text" className={styles.buscadorInput} placeholder="Buscar proveedor por nombre..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
              {busqueda && <button className={styles.buscadorLimpiar} onClick={() => setBusqueda('')}>✕</button>}
            </div>
            {cargando ? (
              <div className={styles.estado}><div className={styles.spinner}></div>Cargando proveedores...</div>
            ) : proveedores.length === 0 ? (
              <div className={styles.estado}>No se encontraron proveedores.</div>
            ) : (
              <table className={styles.tabla}>
                <thead><tr><th>Nombre</th><th>CUIT</th><th>Teléfono</th><th>Email</th><th>Dirección</th><th>Acciones</th></tr></thead>
                <tbody>
                  {proveedores.map(p => (
                    <tr key={p.id} className={styles.tablaFila}>
                      ...
                      <td className={styles.tdAcciones}>
                        ...
                        <button className={styles.btnEliminar} onClick={() => eliminarProveedor(p.id, p.nombre)}>Eliminar</button>
                        {p.tiene_cuenta_corriente && <button className={styles.btnVerCC} onClick={() => verCuentaCorriente(p)}>$ Ver CC</button>}
                        <button className={styles.btnVerHist} onClick={() => verHistorialProveedor(p)}>↓ Compras</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {!proveedorCC ? (
              <div>
                ...
                {cargandoCC ? (
                  ...
                  <div className={styles.estado}>No hay compras registradas con proveedores todavía.<div className={styles.estadoSub}>Registrá una compra desde la sección Compras para que aparezca acá.</div></div>
                ) : (
                  <>
                    <div className={styles.kpiRow}>
                      <div className={styles.kpiCard}><span className={styles.kpiLabel}>Proveedores con deuda</span><span className={styles.kpiValor} style={{ color: '#00C8F0' }}>{proveedoresCC.filter(p => p.saldo > 0).length}</span></div>
                      <div className={styles.kpiCard}><span className={styles.kpiLabel}>Deuda total</span><span className={styles.kpiValor} style={{ color: '#FFB800' }}>{fmtP(proveedoresCC.filter(p => p.saldo > 0).reduce((s, p) => s + p.saldo, 0))}</span></div>
                      <div className={styles.kpiCard}><span className={styles.kpiLabel}>En crítico</span><span className={styles.kpiValor} style={{ color: '#FF4444' }}>{proveedoresCC.filter(p => p.estado === 'critico').length}</span></div>
                      <div className={styles.kpiCard}><span className={styles.kpiLabel}>Total pagado</span><span className={styles.kpiValor} style={{ color: '#00E87A' }}>{fmtP(proveedoresCC.reduce((s, p) => s + p.total_pagos, 0))}</span></div>
                    </div>
                    <table className={styles.tabla}>
                      <thead><tr><th>Proveedor</th><th>CUIT</th><th className={styles.thCenter}>Compras</th><th className={styles.thRight}>Total comprado</th><th className={styles.thRight}>Total pagado</th><th className={styles.thRight}>Saldo</th><th className={styles.thCenter}>Estado</th></tr></thead>
                      <tbody>
                        {proveedoresCC.map(p => (
                          <tr key={p.id} className={styles.tablaFila} onClick={() => seleccionarProveedorCC(p)}>
                            ...
                            <td className={styles.tdRight}><span className={p.saldo > 0 ? styles.saldoDeudor : styles.saldoAFavor}>{p.saldo < 0 ? `+${fmtP(Math.abs(p.saldo))}` : fmtP(p.saldo)}</span></td>
              ...
              <div>
                  ...
                  {detalle && (
                    <div className={styles.detalleSaldoWrap}>
                      ...
                      <span className={`${styles.detalleSaldo} ${detalle.saldo < 0 ? styles.saldoAFavor : detalle.saldo === 0 ? styles.saldoCero : styles.saldoDeudor}`}>
                        {detalle.saldo < 0 ? `+${fmtP(Math.abs(detalle.saldo))} a favor` : detalle.saldo === 0 ? 'Sin deuda' : `Debemos: ${fmtP(detalle.saldo)}`}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className={styles.filtrosWrap}>
                  ...
                  <button className={styles.btnFiltrar} onClick={() => cargarDetalle(proveedorCC, filtroDesde, filtroHasta)}>▶ Filtrar</button>
                  <button className={styles.btnLimpiarFiltro} onClick={() => { setFiltroDesde(''); setFiltroHasta(''); cargarDetalle(proveedorCC) }}>✕ Limpiar</button>
                </div>
                ...
                : detalle && (
                  <div className={styles.detalleGrid}>
                    <div className={styles.detallePanel}>
                      <div className={styles.detallePanelHeader}><span className={styles.panelTitulo}>Compras realizadas</span><span className={styles.panelContador}>{detalle.compras?.length || 0} registros</span></div>
                      {!detalle.compras?.length ? <div className={styles.tablaVacia}>Sin compras en el período</div> : (
                        <table className={styles.tablaInterna}>
                          <thead><tr><th>Número</th><th>Fecha</th><th className={styles.thRight}>Total</th></tr></thead>
                          <tbody>{detalle.compras.map((c, i) => <tr key={i}><td>C-{c.id?.slice(0,8).toUpperCase()}</td><td>{fmtF(c.fecha)}</td><td className={styles.tdRight} style={{ color: '#FFB800' }}>{fmtP(c.total)}</td></tr>)}</tbody>
                          <tfoot><tr><td colSpan={2} className={styles.tfootLabel}>Subtotal compras</td><td className={`${styles.tdRight} ${styles.tfootValor}`}>{fmtP(detalle.compras.reduce((s, c) => s + parseFloat(c.total), 0))}</td></tr></tfoot>
                        </table>
                      )}
                    </div>
                    <div className={styles.detallePanel}>
                      <div className={styles.detallePanelHeader}><span className={styles.panelTitulo}>Pagos realizados</span><span className={styles.panelContador}>{detalle.pagos?.length || 0} registros</span></div>
            ...
            {!proveedorHist ? (
              <div>
                <div className={styles.histSeleccionarMsg}>
                  Seleccioná un proveedor desde la pestaña <strong>Proveedores</strong> usando el botón <span className={styles.inlineBtn}>↓ Compras</span>
                </div>
                <table className={styles.tabla}>
                  ...
                  <tbody>
                    {proveedores.map(p => (
                      <tr key={p.id} className={styles.tablaFila}>
                        <td className={styles.tdNombre}>{p.nombre}</td>
                        <td className={styles.tdSub}>{p.cuit || '—'}</td>
                        <td><button className={styles.btnVerHist} onClick={() => seleccionarProveedorHist(p)}>↓ Ver compras</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div>
                <div className={styles.detalleHeader}>
                  ...
                  <div className={styles.detalleProvInfo}>
                    <span className={styles.detalleNombre}>{proveedorHist.nombre}</span>
                    {proveedorHist.cuit && <span className={styles.detalleSub}>CUIT: {proveedorHist.cuit}</span>}
                  </div>
                  <div className={styles.histResumen}>
                    <span className={styles.histTotal}>{histCompras.length} compras</span>
                    <span className={styles.histMonto}>{fmtP(histCompras.reduce((s, c) => s + parseFloat(c.total), 0))}</span>
                  </div>
                </div>
                ...
                <div className={styles.filtrosWrap}>
                  ...
                  <button className={styles.btnFiltrar} onClick={() => cargarHistorial(proveedorHist.id, histDesde, histHasta)}>▶ Filtrar</button>
                  <button className={styles.btnLimpiarFiltro} onClick={() => { setHistDesde(''); setHistHasta(''); cargarHistorial(proveedorHist.id) }}>✕ Limpiar</button>
                </div>

                {cargandoHist ? (
                  <div className={styles.estado}><div className={styles.spinner}></div>Cargando compras...</div>
                ) : histCompras.length === 0 ? (
                  <div className={styles.estado}>No hay compras registradas para este proveedor en el período.</div>
                ) : (
                  <div className={styles.histLista}>
                    {histCompras.map(c => (
                      <div key={c.id} className={styles.histCard}>
                        <div className={styles.histCardHeader} onClick={() => setCompraDetalle(compraDetalle?.id === c.id ? null : c)}>
                          <div className={styles.histCardLeft}>
                            <span className={styles.histNumero}>{c.numero}</span>
                            <span className={styles.histFecha}>{fmtF(c.fecha)}</span>
                            {c.observaciones && <span className={styles.histObs}>{c.observaciones}</span>}
                          </div>
                          <div className={styles.histCardRight}>
                            <span className={styles.histMonto}>{fmtP(c.total)}</span>
                            <span className={styles.histExpandir}>{compraDetalle?.id === c.id ? '▲ Cerrar' : '▼ Ver productos'}</span>
                          </div>
                        </div>
                        {compraDetalle?.id === c.id && c.items?.length > 0 && (
      ...
      {modalAbierto && (
        <div className={styles.modalOverlay} onClick={cerrarModal}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}><h3>{proveedorEditar ? 'Editar proveedor' : 'Nuevo proveedor'}</h3><button className={styles.modalCerrar} onClick={cerrarModal}>✕</button></div>
            <div className={styles.modalBody}>
              <div className={styles.formGrupo}><label>Nombre / Razón social *</label><input type="text" className={styles.input} placeholder="Ej: Distribuidora San Martín S.A." value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} required /></div>
              ...
              <div className={styles.formGrupo}><label>Email</label><input type="email" className={styles.input} placeholder="proveedor@empresa.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></div>
              ...
              <div className={styles.modalAcciones}>
                ...
                <button className={styles.btnConfirmar} onClick={guardarProveedor}>{proveedorEditar ? '✓ Guardar cambios' : '✓ Crear proveedor'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {modalPago && (
        <div className={styles.modalOverlay} onClick={() => setModalPago(false)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}><h3>Registrar pago a proveedor</h3><button className={styles.modalCerrar} onClick={() => setModalPago(false)}>✕</button></div>
            <div className={styles.modalBody}>
              <div className={styles.modalProv}><span className={styles.modalProvLabel}>Proveedor</span><strong style={{ color: '#00C8F0' }}>{proveedorCC?.nombre}</strong></div>
              ...
              <div className={styles.formGrupo}><label>Medio de pago</label>
                <select className={styles.input} value={pago.medio_pago} onChange={e => setPago({ ...pago, medio_pago: e.target.value })}>
                  <option value="efectivo">Efectivo</option><option value="transferencia">Transferencia bancaria</option><option value="mercado_pago">Mercado Pago</option><option value="debito">Tarjeta de débito</option><option value="credito">Tarjeta de crédito</option><option value="cheque">Cheque</option>
                </select>
              </div>
