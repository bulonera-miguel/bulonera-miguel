"""
pdf_venta.py — Generador de PDF imprimible de Venta
Bulonera Miguel — Formato A4

Genera un comprobante de venta simple (sin CAE/AFIP)
con logo, datos de la empresa y detalle de productos.
"""
...
def generar_pdf_venta(venta: dict) -> bytes:
    """
    Genera PDF imprimible de una venta.

    Parámetro `venta` (dict):
      - numero:        str   → "V-XXXXXXXX"
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Venta {venta.get('numero', '')}",
    ...
    # 1. Encabezado
    story.append(_encabezado(venta, st))
    story.append(Spacer(1, 6*mm))

    # 2. Datos cliente
    story.append(_bloque_cliente(venta, st))
    story.append(Spacer(1, 6*mm))

    # 3. Tabla items
    story.append(_tabla_items(venta, st))
    story.append(Spacer(1, 4*mm))

    # 4. Total
    story.append(_tabla_total(venta, st))
    story.append(Spacer(1, 6*mm))

    # 5. Observaciones
    obs = (venta.get("observaciones") or "").strip()
    if obs:
...
def _encabezado(venta: dict, st: dict) -> Table:
    ...
    fecha  = _fmt_fecha(venta.get("fecha", ""))
    col_der = [
        Paragraph("<font size='7'>COMPROBANTE DE VENTA</font>", st["right_small"]),
...
def _bloque_cliente(venta: dict, st: dict) -> Table:
    cliente = venta.get("cliente") or {}
    nombre  = cliente.get("nombre") or "Consumidor Final"
    cuit    = cliente.get("cuit") or "—"

    filas = [
        [Paragraph("DATOS DEL CLIENTE", st["section_title"]), Paragraph("", st["body"])],
        [Paragraph("Nombre / Razón Social:", st["label"]), Paragraph(nombre, st["body"])],
        [Paragraph("CUIT:", st["label"]),                  Paragraph(cuit,   st["body"])],
    ]
...
def _tabla_items(venta: dict, st: dict) -> Table:
...
def _tabla_total(venta: dict, st: dict) -> Table:
